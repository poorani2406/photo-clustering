    ...
    while True:
        ...
        for face_id in list(candidate_ids):
            ...
            for cand_lbl, profile in cluster_profiles.items():
                ...
                if centroid_sim > best_centroid_sim:
                    best_centroid_sim = centroid_sim
                    best_max_face_sim = max_face_sim
                    best_cluster = cand_lbl
                    best_size = profile["size"]
                    
            # Merge conditions
            centroid_passed = (best_centroid_sim >= CLUSTER_MERGE_THRESHOLD) if best_cluster is not None else False
            face_passed = (best_max_face_sim >= CLUSTER_FACE_THRESHOLD) if best_cluster is not None else False
            merged = centroid_passed and face_passed
            ...
            if merged:
                ...
                print(f"Best cluster selected:\n{best_cluster if best_cluster is not None else 'None'}\n")
                print("Decision:\nREJECTED\n")
                if best_cluster is not None:
                    if not centroid_passed and not face_passed:
                        reason = f"both thresholds failed: centroid similarity ({best_centroid_sim:.6f}) is below threshold ({CLUSTER_MERGE_THRESHOLD}) AND maximum face similarity ({best_max_face_sim:.6f}) is below threshold ({CLUSTER_FACE_THRESHOLD})"
                    elif not centroid_passed:
                        reason = f"centroid threshold failed: centroid similarity ({best_centroid_sim:.6f}) is below threshold ({CLUSTER_MERGE_THRESHOLD})"
                    else:
                        reason = f"face threshold failed: maximum face similarity ({best_max_face_sim:.6f}) is below threshold ({CLUSTER_FACE_THRESHOLD})"
                    print(f"Reason for rejection:\n{reason}\n")
                else:
                    print("Reason for rejection:\nNo candidate clusters available.\n")
